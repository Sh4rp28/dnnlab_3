...
print("\nЗадание 1.\nИсходный тензор x (int32):", x)

# 2. Преобразуйте тензор к типу float32
x = x.to(dtype=torch.float32)  # Преобразуем в float32
print("Тензор x после преобразования в float32:", x)

# 3. Проведите с тензором x ряд операций:
# – возведение в степень n, где n = 2, т.к. номер в списке группы 29
x_pow = x ** 2  # Возведение в квадрат
print("x после возведения в степень 2:", x_pow)

# – умножение на случайное значение в диапазоне от 1 до 10 (через rand)
random_value = torch.rand(1) * 9 + 1  # Случайное значение от 1 до 10
x_mul = x_pow * random_value  # Умножение на случайное значение
print("x после умножения на случайное значение:", x_mul)

# – взятие экспоненты от полученного числа
x_exp = torch.exp(x_mul)  # Экспонента
...
# Для вычисления производной создаем тензор x_exp с requires_grad=True
x_exp = torch.tensor([x_exp.item()], dtype=torch.float32, requires_grad=True)  # Создаем тензор с отслеживанием градиентов

# Вычисляем производную
x_exp.backward()  # Обратный проход для вычисления градиента
print("Производная по x_exp:", x_exp.grad)
